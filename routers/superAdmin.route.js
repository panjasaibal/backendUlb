...
const fetchSuperAdmin = require("../middleware/superadmin.middlware");
...
router.post(
  ...
  [
    ...
    body("password", "Password should be atleast 4 charecters").isLength({
      min: 4,
    }),
  ],
  async (req, res) => {
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (
        superadminUser === null ||
        superadminUser.password !== req.body.password
      ) {
        return res
          .status(400)
...
router.post(
  ...
  [
    ...
    body("passwd", "Password should be atleast 4 charecters").isLength({
      min: 4,
    }),
  ],
  async (req, res) => {
...
router.put("/admin/:id", fetchSuperAdmin, async (req, res) => {
  try {
    const id = req.params.id;
    const { access } = req.body;
    let admin = await Adminstration.findById(id);
    if (admin == null) {
      return res.status(401).send("Access Denied");
    }
    admin = await Adminstration.findByIdAndUpdate(
      id,
      { $set: { access: access } },
      { new: true }
    );
